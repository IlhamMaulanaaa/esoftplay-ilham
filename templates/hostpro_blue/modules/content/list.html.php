<div class="card">
  <h1 class="title_search_result"><?php echo $cat['title']; ?></h1>
  <?php
  if ($cat['total_page'] > 1) 
  {
    ?>
    <span class="text text-muted pull-right text-justify">
      <?php echo lang('page') . ' ' . ($page + 1) . ' ' . lang('of') . ' ' . items($cat['total_page'], 'page'); ?>
    </span>
  ...
  <ul>
    ...
    foreach ((array) $cat['list'] as $data) 
    {
      ...
      <li>
        ...
        <div class="row">
          <?php echo !empty($image) ? '<div class="col-md-4 text-center no-both image">' . $image . '</div>' : ''; ?>
          <div style="width: 1000px;" class="col-md-<?php echo $col; ?> text-justify" class="description">
            <div class="row">
              <?php 
              if (!empty($config['title'])) 
              {
                ...
                if (!empty($config['rating']) || !empty($config['tag'])) 
                { 
                  ?>
                  <div class="col-md-6 text-right tags">
                    <?php $r = content_category($data['id'], $config['tag_link']); echo lang('Tags') . implode(' ', $r);?>
                  </div>
                  <?php
                } 
                ?>
            ...
            <div class="row ">
              ...
              { 
                ...
                { 
                  ?>
                  <div class=" text-left col-md-3 modification">
                ...
                {
                  echo ($config['modified']) ? '<div  class=" text-left col-md-3 modification"><span class="text-muted">' . lang('modified') . content_date($data['modified']) . '</span></div>' : ''; ?>
                <?php
                }
              }
  ...
  { 
    ?>
    <a href="<?php echo $cat['rss']; ?>" class="btn btn-warning btn-sm pull-right"
      title="<?php echo lang('category feed'); ?>">
      <?php echo icon('fa-rss-square'); ?>
      <?php echo lang('category feed'); ?>
    </a>
    <?php   
  } 
  ?>
